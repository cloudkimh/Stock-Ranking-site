  ...
  cron.schedule(
    ...
    async () => {
      ...
      await stockListCronJob("0")
        .then(() =>
          console.log(
            "✅ KOSDAQ stock list cron completed successfully: Execute every 24 Hours"
          )
        )
        .catch((error) =>
          console.error("❌ Error in KOSDAQ stock list cron: ", error)
        );
      ...
      await stockListCronJob("10")
        .then(() =>
          console.log(
            "✅ KOSPI stock list cron completed successfully: Execute every 24 Hours"
          )
        )
        .catch((error) =>
